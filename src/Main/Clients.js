...
const Clients = () => {
    ...
    const getProduct = async (page) => {
      const data = await axios.get(`?get_clients=&name=${search.name}&phoneNumber=${search.phoneNumber}`)
  ...
  return (
    <div>
      <NavLink to='/app/clients/add'  className={classes.link} >
      ...
      <Grid container spacing={1} >
          ...
          <OutlinedInput fullWidth onChange={SearchText}  placeholder="Search by client name"   name='name'     
            startAdornment={<InputAdornment position="start"><SearchIcon color="action"/></InputAdornment>}
          ></OutlinedInput>
          </Grid>
          <Grid item  lg={4} md={4} sm={6} xs={12} >
          <OutlinedInput fullWidth onChange={SearchText}  placeholder="Search by phone number"   name='phoneNumber'     
            startAdornment={<InputAdornment position="start"><SearchIcon color="action"/></InputAdornment>}
          ></OutlinedInput>
          </Grid>
        </Grid>
        ...
        <TableBody>
          {pagination.map((row) => { 
            ...
            <TableRow key={row.id}>
              ...
              <NavLink to={`/app/clients/edit/${row.id}`} >
