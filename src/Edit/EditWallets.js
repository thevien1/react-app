...
const onSubmit = async (values, props) => {
    ...
    var packages = 0
    inputFields.map((item, i)=>{
      packages +=Number(item.price)*Number(item.quantity)/Number(input.price)
    })
    let errorMessage = '';
  ...
  return (
    <div>
        ...
        <Grid container item >
        <Typography className={classes.name} >Update wallet</Typography>
        <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={onSubmit}>
                    {(props) => (
                        <Form autoComplete="off">
                            <TextField onChange={handleinput}  type='number' value={input.orderNumber} name='orderNumber' error={Boolean(input.orderNumber==='' ) } helperText={input.orderNumber===''  ? 'No is required': ''} label="No" size="small" variant="outlined"fullWidth/> 
                          ...
                          <TextField onChange={handleinput} className={classes.fontText} type='number' value={formattedPrice} name='price' error={Boolean(input.price==='' ) } helperText={input.price===''  ? 'Price is required': ''} label="Price" size="small" variant="outlined"fullWidth/> 
                          <TextField onChange={handleinput}  type='number' value={formattedAmount} name='amount' error={Boolean(input.amount==='' ) } helperText={input.amount===''  ? 'Dollar is required': ''} label="Price in Dollar" size="small" variant="outlined"fullWidth/> 
                          ...
                          <div className={classes.budget} key={inputField.id}>
                              ...
                              <TextField className={clsx(classes.margin, classes.textField)} type='number' name='price' value={Number.isNaN(parseFloat(inputField.price)) ? '' : Number.parseFloat(inputField.price)} onChange={event => handleChangeInput(inputField.id, event)} label="Price"  variant="outlined"  size="small"
                                    InputProps={{
                                      endAdornment: (
                                        <div>
                        ...
                        <br/>
<TextField label="Notes"  multiline minRows='4' onChange={handleinput} name='notes' value={input.notes}  className={classes.fontText}  variant="outlined" fullWidth/>
